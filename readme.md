...
The app generates a unique language based on your parameters using a dictionary of ~61000 common English words for reference. All words generated should be able to be pronounced because word structures are hardcoded, meaning generated languages can be spoken as well. Language structure will remain the same as English, but it will be unintelligible to anyone, because it generally shares no similarities to any other existing languages. You CAN customize the language it generates to your liking, adding words that ARE similar to existing languages for nicer flow, if you would like.

In comparison to real-life languages, the languages generated by PyLangGen will be extremely complex. Every state of a word (past, present, future, plural, infinitive, accusative, etc) will be completely different to the others. All words will share no similarily in relation to meaning. This will make learning the languages extremely hard, but with the modification of generated languages (a feature in-app), this could be worked around if necessary.
...
All languages you generate can be saved as .lang files to be reloaded for future translation.
...
*DO NOT RUN THE APP AS ADMIN. When batch files are run as admin, they are executed within the System32 directory which this app is not intended to handle.
